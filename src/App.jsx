...
return { …DEFAULT_DATA, …parsed, categories: migratedCategories, accounts: parsed.accounts || DEFAULT_ACCOUNTS, defaultAccount: parsed.defaultAccount || “Main”, paySchedule: parsed.paySchedule || DEFAULT_DATA.paySchedule };
...
const ytdStats = (() => {
const schedule = data.paySchedule || {};
if (!schedule.amount) return null;
const todayStr = today();
const yr = new Date().getFullYear();
const loggedYtd = data.paychecks.filter(p => p.date.startsWith(String(yr)) && p.date <= todayStr).reduce((s, p) => s + p.amount, 0);
let scheduledReceived = 0;
if (schedule.firstPayDate) {
const allDates = getPaycheckDates(yr);
const loggedDates = new Set(data.paychecks.map(p => p.date));
const unloggedCount = allDates.filter(d => { const ds = d.toISOString().split(“T”)[0]; return ds <= todayStr && !loggedDates.has(ds); }).length;
scheduledReceived = unloggedCount * (schedule.amount || 0);
}
const ytdReceived = loggedYtd + scheduledReceived;
...
const firstPayDate = schedule.firstPayDate;
if (!firstPayDate) return [];
const firstDate = new Date(firstPayDate + “T00:00:00”);
const dates = [];
const yearStart = new Date(year + “-01-01T00:00:00”);
const yearEnd = new Date(year + “-12-31T00:00:00”);
const effectiveStart = firstDate > yearStart ? firstDate : yearStart;
if (freq === “semimonthly”) {
for (let m = 0; m < 12; m++) {
const d1 = new Date(year, m, 1), d2 = new Date(year, m, 15);
if (d1 >= effectiveStart && d1 <= yearEnd) dates.push(d1);
if (d2 >= effectiveStart && d2 <= yearEnd) dates.push(d2);
}
return dates;
}
if (freq === “monthly”) {
const dayOfMonth = firstDate.getDate();
for (let m = 0; m < 12; m++) { const d = new Date(year, m, dayOfMonth); if (d >= effectiveStart && d <= yearEnd) dates.push(d); }
return dates;
}
let intervalDays = freq === “weekly” ? 7 : freq === “custom” ? (schedule.customDays || 14) : 14;
let cursor = new Date(firstDate);
while (cursor < yearStart) cursor = new Date(cursor.getTime() + intervalDays * 86400000);
while (cursor <= yearEnd) { if (cursor >= effectiveStart) dates.push(new Date(cursor)); cursor = new Date(cursor.getTime() + intervalDays * 86400000); }
return dates;
}
  ...
  <main style={{ flex: 1, padding: "20px 16px", maxWidth: 600, margin: "0 auto", width: "100%", boxSizing: "border-box", paddingBottom: 100 }}>
    ...
    {view === "budgets" && (() => {
      ...
      const monthlyPay = (() => {
        ...
        if (freq === "custom")      return amt * (365 / (schedule.customDays || 14)) / 12;
        return amt * 26 / 12;
      })();
    ...
    {view === "settings" && (
      <div>
        ...
        <div style={{ ...S.card, padding: 0, overflow: "hidden", marginBottom: 12 }}>
          ...
          {payScheduleExpanded && (
            <div style={{ borderTop: "1px solid #ddd8d0", padding: 16 }}>
              <div style={{ fontSize: 11, letterSpacing: 2, textTransform: "uppercase", fontFamily: "'Righteous', cursive", color: "#7a736a", marginBottom: 12 }}>Your Regular Paycheck</div>
              <div style={{ marginBottom: 14 }}>
                <label style={S.label}>Default Take-Home Amount ($)</label>
                <input style={S.input} type="number"
                  value={(data.paySchedule || {}).amount || ""}
                  onChange={e => setData(d => ({ ...d, paySchedule: { ...d.paySchedule, amount: parseFloat(e.target.value) || 0 } }))}
                  placeholder="e.g. 1500" />
                <div style={{ fontSize: 11, color: "#9a9088", marginTop: 6 }}>We'll use this when you haven't logged a paycheck yet.</div>
              </div>
              <div style={{ marginBottom: 14 }}>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                  {[["biweekly","Every 2 weeks"],["semimonthly","1st & 15th"],["monthly","Monthly"],["weekly","Weekly"],["custom","Custom"]].map(([id, label]) => (
                    <button key={id} onClick={() => setData(d => ({ ...d, paySchedule: { ...d.paySchedule, frequency: id } }))}
                      style={{ padding: "7px 12px", borderRadius: 20, border: "1px solid", borderColor: (data.paySchedule || {}).frequency === id ? "#e07b54" : "#ddd8d0", background: (data.paySchedule || {}).frequency === id ? "#e07b5422" : "transparent", color: (data.paySchedule || {}).frequency === id ? "#e07b54" : "#7a736a", fontSize: 12, cursor: "pointer", fontFamily: "inherit" }}>
                      {label}
                    </button>
                  ))}
                </div>
                {(data.paySchedule || {}).frequency === "custom" && (
                  <div style={{ marginTop: 8 }}>
                    <label style={S.label}>Every how many days?</label>
                    <input style={S.input} type="number"
                      value={(data.paySchedule || {}).customDays || ""}
                      onChange={e => setData(d => ({ ...d, paySchedule: { ...d.paySchedule, customDays: parseFloat(e.target.value) || 14 } }))}
                      placeholder="e.g. 10" />
                  </div>
                )}
              </div>
              <div style={{ marginBottom: 20 }}>
                ...
                <input style={S.input} type="date"
                  value={(data.paySchedule || {}).firstPayDate || ""}
                  onChange={e => setData(d => ({ ...d, paySchedule: { ...d.paySchedule, firstPayDate: e.target.value } }))} />
                <div style={{ fontSize: 11, color: "#9a9088", marginTop: 6 }}>We'll use this to figure out all your future pay dates.</div>
              </div>

              <div style={{ paddingTop: 16, borderTop: "1px solid #ddd8d0" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                ...
                {paycheckLogExpanded && (
                  <>
                    <div style={{ fontSize: 11, color: "#9a9088", marginBottom: 10 }}>Log real paychecks here to override the default. Got a bonus? Add it too 🎉</div>
                    {data.paychecks.length === 0 ? (
                      <div style={{ textAlign: "center", color: "#9a9088", padding: "12px 0", fontSize: 13 }}>No paychecks logged yet — we'll use your default amount until you do.</div>
                    ) : (
                      <>
                        ...
                        {(() => {
                          ...
                          return filtered.length === 0 ? (
                            ...
                            <>
                              ...
                              {total > 12 && (
                                <div style={{ fontSize: 11, color: "#9a9088", textAlign: "center", paddingTop: 10 }}>Showing 12 of {total} paychecks for {paycheckLogYear}</div>
                              )}
                            </>
                          );
                        })()}
