...
const [dataMgmtExpanded, setDataMgmtExpanded] = useState(false);
const [dataMgmtConfirm, setDataMgmtConfirm] = useState(null); // { type, count, ids }
const [dmExpFilter, setDmExpFilter] = useState({ years: [], months: [], cats: [], mode: “month” });
const [dmPayFilter, setDmPayFilter] = useState({ years: [] });
  ...
  {dataMgmtConfirm && (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", zIndex: 300, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
      <div style={{ background: "#ffffff", borderRadius: 16, padding: 24, maxWidth: 340, width: "100%", border: "1px solid #ddd8d0" }}>
        ...
        <div style={{ fontSize: 15, fontWeight: "bold", color: "#2a2520", marginBottom: 12, textAlign: "center" }}>Whoa there!</div>
        <div style={{ fontSize: 13, color: "#7a736a", marginBottom: 20, lineHeight: 1.6, textAlign: "center" }}>
          {dataMgmtConfirm.count} {dataMgmtConfirm.type === "expenses" ? "transaction" : "paycheck"}{dataMgmtConfirm.count !== 1 ? "s" : ""} are about to meet their end. This is permanent — no undo, no take-backs. You sure? 👀
        </div>
        <div style={{ display: "flex", gap: 10 }}>
          <button onClick={() => {
            const ids = new Set(dataMgmtConfirm.ids);
            if (dataMgmtConfirm.type === "expenses") {
              setData(d => ({ ...d, expenses: d.expenses.filter(e => !ids.has(e.id)) }));
              setToast("Poof! " + dataMgmtConfirm.count + " transactions gone 🗑️");
            } else {
              setData(d => ({ ...d, paychecks: d.paychecks.filter(p => !ids.has(p.id)) }));
              setToast("Poof! " + dataMgmtConfirm.count + " paychecks gone 🗑️");
            }
            setDataMgmtConfirm(null);
          }}
            style={{ flex: 1, padding: 13, background: "#c94040", border: "none", borderRadius: 10, color: "#ffffff", fontSize: 13, fontWeight: "bold", cursor: "pointer", fontFamily: "inherit" }}>
            Delete {dataMgmtConfirm.count}
          </button>
          <button onClick={() => setDataMgmtConfirm(null)}
            style={{ flex: 1, padding: 13, background: "transparent", border: "1px solid #ddd8d0", borderRadius: 10, color: "#7a736a", fontSize: 13, cursor: "pointer", fontFamily: "inherit" }}>
            Cancel
          </button>
        </div>
      </div>
  ...
  <main style={{ flex: 1, padding: "20px 16px", maxWidth: 600, margin: "0 auto", width: "100%", boxSizing: "border-box", paddingBottom: 100 }}>
    ...
    {view === "settings" && (
      <div>
        ...
        {/* Data Management */}
        <div style={{ ...S.card, padding: 0, overflow: "hidden", marginBottom: 12 }}>
          <button onClick={() => setDataMgmtExpanded(p => !p)}
            style={{ width: "100%", padding: "14px 16px", background: "transparent", border: "none", color: "#2a2520", cursor: "pointer", fontFamily: "inherit", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <span style={{ fontSize: 11, letterSpacing: 3, textTransform: "uppercase", fontFamily: "'Righteous', cursive", color: "#7a736a" }}>Data Management</span>
              <span style={{ fontSize: 11, color: "#9a9088" }}>Bulk delete transactions or paychecks</span>
            </div>
            <ChevronDown style={{ transition: "transform 0.25s ease", transform: dataMgmtExpanded ? "rotate(180deg)" : "rotate(0deg)", flexShrink: 0 }} />
          </button>

          {dataMgmtExpanded && (() => {
            const allYears = [...new Set([...data.expenses.map(e => e.date.slice(0,4)), ...data.paychecks.map(p => p.date.slice(0,4))])].sort().reverse();
            const expYears = [...new Set(data.expenses.map(e => e.date.slice(0,4)))].sort().reverse();
            const payYears = [...new Set(data.paychecks.map(p => p.date.slice(0,4)))].sort().reverse();

            // Compute filtered expenses
            const filteredExp = data.expenses.filter(e => {
              const yrMatch = dmExpFilter.years.length === 0 || dmExpFilter.years.includes(e.date.slice(0,4));
              const moMatch = dmExpFilter.mode === "year" || dmExpFilter.months.length === 0 || dmExpFilter.months.includes(getMonthKey(e.date));
              const catMatch = dmExpFilter.cats.length === 0 || (e.splits && e.splits.length > 0 ? e.splits.some(s => dmExpFilter.cats.includes(s.category)) : dmExpFilter.cats.includes(e.category));
              return yrMatch && moMatch && catMatch;
            });

            // Compute filtered paychecks
            const filteredPay = data.paychecks.filter(p => dmPayFilter.years.length === 0 || dmPayFilter.years.includes(p.date.slice(0,4)));

            return (
              <div style={{ borderTop: "1px solid #ddd8d0", padding: 16 }}>

                {/* TRANSACTIONS SECTION */}
                <div style={{ fontSize: 11, letterSpacing: 2, textTransform: "uppercase", fontFamily: "'Righteous', cursive", color: "#7a736a", marginBottom: 12 }}>Transactions</div>

                {/* Bulk presets */}
                <div style={{ marginBottom: 14 }}>
                  <div style={{ fontSize: 11, color: "#9a9088", marginBottom: 8 }}>Quick delete by year:</div>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                    {expYears.map(yr => {
                      const count = data.expenses.filter(e => e.date.startsWith(yr)).length;
                      return (
                        <button key={yr} onClick={() => setDataMgmtConfirm({ type: "expenses", count, ids: data.expenses.filter(e => e.date.startsWith(yr)).map(e => e.id), label: "transactions from " + yr })}
                          style={{ padding: "6px 14px", borderRadius: 10, border: "1px solid #c9404044", background: "#c9404011", color: "#c94040", fontSize: 12, cursor: "pointer", fontFamily: "inherit" }}>
                          🗑️ {yr} ({count})
                        </button>
                      );
                    })}
                    {data.expenses.length > 0 && (
                      <button onClick={() => setDataMgmtConfirm({ type: "expenses", count: data.expenses.length, ids: data.expenses.map(e => e.id), label: "ALL transactions" })}
                        style={{ padding: "6px 14px", borderRadius: 10, border: "1px solid #c9404088", background: "#c9404022", color: "#c94040", fontSize: 12, fontWeight: "bold", cursor: "pointer", fontFamily: "inherit" }}>
                        🗑️ All ({data.expenses.length})
                      </button>
                    )}
                  </div>
                </div>
                ...
                {/* Filter & delete */}
                <div style={{ background: "#f0ede8", borderRadius: 10, padding: 14, marginBottom: 20 }}>
                  <div style={{ fontSize: 11, color: "#9a9088", marginBottom: 10 }}>Filter then delete:</div>

                  {/* Year filter */}
                  <div style={{ marginBottom: 10 }}>
                    <div style={{ fontSize: 10, letterSpacing: 2, textTransform: "uppercase", fontFamily: "'Righteous', cursive", color: "#9a9088", marginBottom: 6 }}>Year</div>
                    <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                      {expYears.map(yr => {
                        const sel = dmExpFilter.years.includes(yr);
                        return <button key={yr} onClick={() => setDmExpFilter(p => ({ ...p, years: sel ? p.years.filter(y => y !== yr) : [...p.years, yr], months: [] }))}
                          style={{ padding: "4px 12px", borderRadius: 12, border: "1px solid", borderColor: sel ? "#e07b54" : "#ddd8d0", background: sel ? "#e07b5422" : "transparent", color: sel ? "#e07b54" : "#9a9088", fontSize: 11, cursor: "pointer", fontFamily: "inherit" }}>{yr}</button>;
                      })}
                    </div>
                  </div>
                  ...
                  <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
                    {[["month","By Month"],["year","By Year"]].map(([mode, label]) => (
                      <button key={mode} onClick={() => setDmExpFilter(p => ({ ...p, mode }))}
                        style={{ flex: 1, padding: "6px 0", borderRadius: 8, border: "1px solid", borderColor: dmExpFilter.mode === mode ? "#e07b54" : "#ddd8d0", background: dmExpFilter.mode === mode ? "#e07b5422" : "transparent", color: dmExpFilter.mode === mode ? "#e07b54" : "#9a9088", fontSize: 11, cursor: "pointer", fontFamily: "inherit" }}>
                        {label}
                      </button>
                    ))}
                  </div>
                  ...
                  {/* Month grid */}
                  {dmExpFilter.mode === "month" && dmExpFilter.years.length > 0 && (
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(6,1fr)", gap: 5, marginBottom: 10 }}>
                      {["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"].map((lbl, i) => {
                        const keys = dmExpFilter.years.map(yr => yr + "-" + String(i+1).padStart(2,"0"));
                        const sel = keys.some(k => dmExpFilter.months.includes(k));
                        return <button key={lbl} onClick={() => setDmExpFilter(p => ({ ...p, months: sel ? p.months.filter(m => !keys.includes(m)) : [...p.months, ...keys.filter(k => !p.months.includes(k))] }))}
                          style={{ padding: "5px 0", borderRadius: 6, border: "1px solid", borderColor: sel ? "#e07b54" : "#ddd8d0", background: sel ? "#e07b5422" : "transparent", color: sel ? "#e07b54" : "#9a9088", fontSize: 11, cursor: "pointer", fontFamily: "inherit" }}>{lbl}</button>;
                      })}
                    </div>
                  )}
                  ...
                  <div style={{ marginBottom: 12 }}>
                    <div style={{ fontSize: 10, letterSpacing: 2, textTransform: "uppercase", fontFamily: "'Righteous', cursive", color: "#9a9088", marginBottom: 6 }}>Category</div>
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                      {categories.map(cat => {
                        const sel = dmExpFilter.cats.includes(cat.name);
                        return <button key={cat.name} onClick={() => setDmExpFilter(p => ({ ...p, cats: sel ? p.cats.filter(c => c !== cat.name) : [...p.cats, cat.name] }))}
                          style={{ padding: "4px 9px", borderRadius: 12, border: "1px solid", borderColor: sel ? cat.color : "#ddd8d0", background: sel ? cat.color + "22" : cat.color + "0f", color: sel ? cat.color : "#9a9088", fontSize: 11, cursor: "pointer", fontFamily: "inherit" }}>
                          {cat.icon} {cat.name}
                        </button>;
                      })}
                    </div>
                  ...
                  {/* Filtered count + delete button */}
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <span style={{ fontSize: 12, color: "#9a9088" }}>{filteredExp.length} transaction{filteredExp.length !== 1 ? "s" : ""} match</span>
                    <button onClick={() => filteredExp.length > 0 && setDataMgmtConfirm({ type: "expenses", count: filteredExp.length, ids: filteredExp.map(e => e.id), label: "filtered transactions" })}
                      disabled={filteredExp.length === 0}
                      style={{ padding: "7px 14px", borderRadius: 10, border: "1px solid #c9404055", background: filteredExp.length > 0 ? "#c9404022" : "transparent", color: filteredExp.length > 0 ? "#c94040" : "#ddd8d0", fontSize: 12, cursor: filteredExp.length > 0 ? "pointer" : "not-allowed", fontFamily: "inherit", fontWeight: "bold" }}>
                      🗑️ Delete {filteredExp.length > 0 ? filteredExp.length : ""}
                    </button>
                  </div>
                </div>

                {/* PAYCHECKS SECTION */}
                <div style={{ fontSize: 11, letterSpacing: 2, textTransform: "uppercase", fontFamily: "'Righteous', cursive", color: "#7a736a", marginBottom: 12 }}>Bonus Pay</div>
                ...
                <div style={{ marginBottom: 14 }}>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                    {payYears.map(yr => {
                      const count = data.paychecks.filter(p => p.date.startsWith(yr)).length;
                      return (
                        <button key={yr} onClick={() => setDataMgmtConfirm({ type: "paychecks", count, ids: data.paychecks.filter(p => p.date.startsWith(yr)).map(p => p.id), label: "paychecks from " + yr })}
                          style={{ padding: "6px 14px", borderRadius: 10, border: "1px solid #c9404044", background: "#c9404011", color: "#c94040", fontSize: 12, cursor: "pointer", fontFamily: "inherit" }}>
                          🗑️ {yr} ({count})
                        </button>
                      );
                    ...
                    {data.paychecks.length > 0 && (
                      <button onClick={() => setDataMgmtConfirm({ type: "paychecks", count: data.paychecks.length, ids: data.paychecks.map(p => p.id), label: "ALL paychecks" })}
                        style={{ padding: "6px 14px", borderRadius: 10, border: "1px solid #c9404088", background: "#c9404022", color: "#c94040", fontSize: 12, fontWeight: "bold", cursor: "pointer", fontFamily: "inherit" }}>
                        🗑️ All ({data.paychecks.length})
                      </button>
                    )}
                  </div>
                </div>

                {/* Filter & delete paychecks */}
                <div style={{ background: "#f0ede8", borderRadius: 10, padding: 14 }}>
                  <div style={{ fontSize: 11, color: "#9a9088", marginBottom: 10 }}>Filter then delete:</div>
                  <div style={{ fontSize: 10, letterSpacing: 2, textTransform: "uppercase", fontFamily: "'Righteous', cursive", color: "#9a9088", marginBottom: 6 }}>Year</div>
                  <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 12 }}>
                    {payYears.map(yr => {
                      const sel = dmPayFilter.years.includes(yr);
                      return <button key={yr} onClick={() => setDmPayFilter(p => ({ ...p, years: sel ? p.years.filter(y => y !== yr) : [...p.years, yr] }))}
                        style={{ padding: "4px 12px", borderRadius: 12, border: "1px solid", borderColor: sel ? "#2e9e5e" : "#ddd8d0", background: sel ? "#2e9e5e22" : "transparent", color: sel ? "#2e9e5e" : "#9a9088", fontSize: 11, cursor: "pointer", fontFamily: "inherit" }}>{yr}</button>;
                    })}
                  </div>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <span style={{ fontSize: 12, color: "#9a9088" }}>{filteredPay.length} paycheck{filteredPay.length !== 1 ? "s" : ""} match</span>
                    <button onClick={() => filteredPay.length > 0 && setDataMgmtConfirm({ type: "paychecks", count: filteredPay.length, ids: filteredPay.map(p => p.id), label: "filtered paychecks" })}
                      disabled={filteredPay.length === 0}
                      style={{ padding: "7px 14px", borderRadius: 10, border: "1px solid #c9404055", background: filteredPay.length > 0 ? "#c9404022" : "transparent", color: filteredPay.length > 0 ? "#c94040" : "#ddd8d0", fontSize: 12, cursor: filteredPay.length > 0 ? "pointer" : "not-allowed", fontFamily: "inherit", fontWeight: "bold" }}>
                      🗑️ Delete {filteredPay.length > 0 ? filteredPay.length : ""}
                    </button>
                  </div>
                </div>
              </div>
